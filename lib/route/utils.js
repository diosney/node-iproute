  ...
  if (!raw_data) {
    ...
    for (var line = 0, output_length = output.length - 1;
      ...
      if (route_fields.length % 2 != 0 ||
          (route_fields.length % 2 == 0 && Object.keys(exports.nhflags).find(n => n === route_fields[route_fields.length -1]))) {
      ...
      for (var i = 0, rest_line_fields_length = rest_line_fields.length;
        ...
        if (Object.keys(exports.nhflags).find(n => n === rest_line_fields[i])) {
          route['nhflag'] = rest_line_fields[i];
        } else {
