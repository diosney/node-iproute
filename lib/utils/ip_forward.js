...
function sysctl(options, cb) {
	...
	switch (options.action) {
		case 'enable':
			args = args.concat('-w', options.path + '=1'); // It can not be --write since is does not supported in older kernels.

			break;
		case 'disable':
			args = args.concat('-w', options.path + '=0'); // It can not be --write since is does not supported in older kernels.
